...
export function FoodScanSheet({ open, onClose, logTimestamp, dateHint, onLogged }: {
  ...
  const add = async () => {
    ...
    try {
      ...
      toast.success(t("Logged {label} · {kcal} kcal", { label: result.label, kcal: result.kcal }));
      close();
      onLogged();
    } catch (err) {
      toast.error((err as Error).message);
      ...
      <div className="w-full sm:max-w-md max-h-[92vh] overflow-y-auto rounded-t-3xl sm:rounded-3xl bg-card border border-border/60 shadow-[var(--shadow-card)]">
        <div className="sticky top-0 z-10 flex items-center justify-between px-4 py-3 bg-card/95 backdrop-blur border-b border-border/40">
          <span className="text-[11px] uppercase tracking-widest text-sand flex items-center gap-1.5">
            <Camera size={13} /> {t("Scan food")}
          </span>
          <button onClick={close} className="p-1.5 rounded-full hover:bg-muted/50" aria-label={t("Close")}>
            <X size={16} />
          </button>
        </div>
        ...
        <div className="p-4 space-y-3">
          ...
          {imageDataUrl ? (
            <div className="relative">
              <img src={imageDataUrl} alt={t("Food to analyse")} className="w-full rounded-2xl object-cover max-h-64" />
          ...
          <textarea
            ...
            placeholder={t('Optional notes — e.g. "grilled chicken breast, cooked in olive oil, no rice"')}
            ...
            <div className="rounded-2xl border border-border/50 bg-muted/20 p-3 space-y-2">
              <div className="flex items-start justify-between gap-2">
                ...
                <span className={`shrink-0 text-[9px] uppercase tracking-wider px-2 py-0.5 rounded-full border ${
                  ...
                  {t("{level} confidence", { level: result.confidence })}
                </span>
              </div>
              ...
              <ul className="space-y-1">
                {result.items.map((it, i) => (
                  <li key={i} className="text-[11px] leading-snug text-muted-foreground">
                    ...
                    {it.grams ? ` (${it.grams}g)` : ""}: {it.kcal} {t("kcal")} · {it.protein_g}{t("P")} / {it.carbs_g}{t("C")} / {it.fat_g}{t("F")}
                    {(it.saturated_fat_g > 0 || it.sugar_g > 0 || it.fiber_g > 0 || it.sodium_mg > 0 || it.animal_protein_g > 0 || it.plant_protein_g > 0) && (
                      <span className="block text-[10px] text-muted-foreground/70">
                        {[
                          it.saturated_fat_g > 0 && t("{n}g sat fat", { n: it.saturated_fat_g }),
                          (it.monounsaturated_fat_g + it.polyunsaturated_fat_g) > 0 && t("{n}g unsat fat", { n: it.monounsaturated_fat_g + it.polyunsaturated_fat_g }),
                          it.sugar_g > 0 && t("{n}g sugar", { n: it.sugar_g }),
                          it.fiber_g > 0 && t("{n}g fiber", { n: it.fiber_g }),
                          it.starch_g > 0 && t("{n}g starch", { n: it.starch_g }),
                          it.sodium_mg > 0 && t("{n}mg sodium", { n: it.sodium_mg }),
                          it.animal_protein_g > 0 && t("{n}g animal protein", { n: it.animal_protein_g }),
                          it.plant_protein_g > 0 && t("{n}g plant protein", { n: it.plant_protein_g }),
                        ].filter(Boolean).join(" · ")}
                      </span>
                    )}
                  </li>
              ...
              <div className="pt-2 border-t border-border/40 grid grid-cols-4 gap-2 text-center">
                {[
                  [t("Kcal"), `${result.kcal}`],
                  [t("Protein"), `${result.protein_g}g`],
                  [t("Carbs"), `${result.carbs_g}g`],
                  [t("Fat"), `${result.fat_g}g`],
                  [t("Animal P"), `${result.animal_protein_g}g`],
                  [t("Plant P"), `${result.plant_protein_g}g`],
                  [t("Sugar"), `${result.sugar_g}g`],
                  [t("Fiber"), `${result.fiber_g}g`],
                  [t("Sat fat"), `${result.saturated_fat_g}g`],
                  [t("Unsat fat"), `${result.monounsaturated_fat_g + result.polyunsaturated_fat_g}g`],
                  [t("Sodium"), `${result.sodium_mg}mg`],
                  [t("Grams"), `${result.total_grams}g`],
                ].map(([k, v]) => (
                  <div key={k}>
              ...
              {userTurns.length > 0 && (
                <div className="pt-2 border-t border-border/40 space-y-1">
                  {userTurns.map((turn, i) => (
                    <div key={i} className="text-[10px] text-sand">↳ {t("you")}: {turn.content}</div>
                  ))}
                </div>
              )}
              ...
              <div className="flex items-center gap-2 pt-1">
                <input
                  ...
                  placeholder={t('Correct it — e.g. "I used 2 tbsp olive oil"')}
                ...
                <button
                  ...
                  className="p-2 rounded-full bg-muted/60 disabled:opacity-40" aria-label={t("Send correction")}>
          ...
          {result && (
            <>
              <label className="flex items-center gap-2 text-xs text-muted-foreground">
                ...
                />
                <Bookmark size={13} /> {t("Save to my foods for one-tap logging later")}
              </label>
              {saveToLibrary && (
                <input
                  value={saveName}
